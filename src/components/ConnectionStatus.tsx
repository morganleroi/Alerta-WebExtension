...
const ConnectionStatus = (props: { fetchAlertStatus: FetchAlertState }) => {
    ...
    return (
        <p>Connection status : {status && status === "OK" ?
            <span>
            ...
            <span>
                ...
                <span> Oups, we're not able to get alerts from Alerta API {error?.status ? `(${error?.status} - ${error?.statusText})` : "(You need to allow the extension to call Alerta API. Please fulfill the form then Save preferences)"}</span>
            </span>}
        </p>
    );
}
